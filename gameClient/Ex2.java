...
public class Ex2 implements Runnable{
	...
	private void init(game_service game) {
		...
		try {
			...
			System.out.println("list = :"+listOfPokemons);//-->prints the arraylist of poks
			for(int i=0;i<listOfPokemons.size();i++) {
				Arena.updateEdge(listOfPokemons.get(i), graph);
				//System.out.println("Edge "+i+":"+listOfPokemons.get(i).);
				//System.out.println("Edge "+i+":"+listOfPokemons.get(i).get_edge());		
			}
			...
			for(int j = 0 ; j<numberOfAgents ; j++) {
				...
				System.out.println(tmp.get_edge());
				game.addAgent(positionOfAgent);
				System.out.println("posison :"+positionOfAgent);
			}
	...
	private static  void moveAgants(game_service game, directed_weighted_graph graph ) throws JSONException  {
		...
		System.out.println("gameMove : "+gameM);
	...
	public static  void setingTheHashMaps(game_service game, directed_weighted_graph graph,String str) {
		...
		System.out.println("List after sorted : " +listOfPokemons);
		...
		for(int i = 0 ; i<agents.size();i++) {
			...
			System.out.println("GETSRCNODE : "+src);
			...
			for(CL_Pokemon cla : listOfPokemons) {
				if(flag2 == false && vis.get(cla) == 0 && cla.get_edge().getSrc() == last) {
					...
					if(last == source) {
						...
						System.out.println("graph.getNode(destination) :"+graph.getNode(destination));
						flag2 = true;
					}
